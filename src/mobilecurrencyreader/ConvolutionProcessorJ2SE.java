                ...
                for (int i1=i-1; i1<=i+1; i1++) {
                    for (int j1=j-1; j1<=j+1; j1++) {
                        if (!(j1 == j && i1 == i) && i1>=0 && j1 >=0 && i1<img.height && j1 < img.width) {
                            total += img.getPixelInt(i1-i+1, j1-j+1) * dx[i1-i+1][j1-j+1] + img.getPixelInt(i1-i+1, j1-j+1) * dy[i1-i+1][j1-j+1];
                            num++;
                        }
                    }
                }
                float avg = ((float)total / (float) num / 2.0f) + 128.0f;
                
                out.setPixel(i, j, (byte)(avg + .5));
                if (i> 60 && i<90 && j> 0 && j<50) {
                    int asdgdsagdsgasd = 9;
                    System.out.println("AVG: " + avg + " TOT: " + total + " NUM: " + num + " NEW: " + out.getPixelInt(i, j) + " ORIG: " + img.getPixelInt(i, j));
                }
